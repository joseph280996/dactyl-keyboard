...
def load_config():
    ...
    if len(sys.argv) <= 1:
        ...
        for opt, arg in opts:
            if opt in ("--config"):
                with open(
                    os.path.join(r"..", "configs", arg + ".json"), mode="r"
                ) as fid:
                    data = json.load(fid)
...
def get_oled_conf():
    ...
    if (
        shape_config.oled_mount_type is not None
        and shape_config.oled_mount_type != "NONE"
    ):
        return shape_config.oled_configurations[shape_config.oled_mount_type]
